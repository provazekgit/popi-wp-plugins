	...
	public static function render_page(): void {
		...
		<div class="wrap" style="max-width:860px;">
			...
			<div class="popi-quickstart">
				...
				<h2>Rychlý start</h2>
				<ol>
					<li>Přidej novou <strong>Landing Page</strong> — vyplň H1 a perex tak, aby přesně odpovídaly textu reklamy (message match).</li>
					<li>Vyplň <strong>CTA — text/URL</strong> a sekci <strong>SEA & Kampaň</strong> (UTM source/medium/campaign).</li>
					<li>Vyplň <strong>SEO & Sdílení</strong> (meta title, meta description, OG obrázek).</li>
					<li>Pokud má LP ceník, vyplň <strong>Ceník</strong> — zobrazí se i ve Schema.org datech.</li>
					<li>Pro stejnou LP na víc platforem (Sklik + Google Ads) použij Duplicate Post a změň jen slug a UTM source.</li>
				</ol>
			</div>
			...
			<?php self::section( 'Pro vývoj a design — Bricks / Beaver Builder + Astra', array(
				'Bricks (doporučeno)' => 'Plugin má pro Bricks vlastní skupinu dynamických dat „Popi" (viz sekce výše) — dynamic tagy pro H1, perex, CTA URL s UTM, meta title, OG obrázek.',
				'Beaver Builder + Astra' => 'Plugin nemá vlastní BB modul ani dynamic tags pro Beaver Builder. Pole se vypisují přes <strong>PHP/Code modul</strong>, např. <code>&lt;?php echo popi_lp_cta_url( get_the_ID() ); ?&gt;</code> nebo <code>&lt;?php the_field(\'popi_lp_hlavni_nadpis\'); ?&gt;</code>. Ceník lze vložit i bez PHP — shortcode <code>[popi_lp_pricing]</code> v HTML/Shortcode modulu.',
				'Hooky a filtry' => 'Všechny <code>popi_lp_*()</code> funkce (CTA, ceník, UTM) lze volat odkudkoliv (PHP/Code modul, vlastní šablona v tématu) bez závislosti na konkrétním builderu.',
			) ); ?>
